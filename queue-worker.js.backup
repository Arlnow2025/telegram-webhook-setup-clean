...
let sessions_send = null;

// CLI wrapper for mcporter (since module has export issues)
const mcporter = {
  call: function(tool, args) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        const mcporterPath = '/root/.nvm/versions/node/v22.22.0/bin/mcporter';
        const cmd = `${mcporterPath} call ${tool} --args '${JSON.stringify(args)}'`;
        ...
        if (!out) throw new Error('Empty response from mcporter');
        ...
        lastError = e;
        console.error(`[MCP] Attempt ${attempt} failed: ${e.message}`);
        if (e.code === 'ETIMEDOUT' && attempt < maxRetries) {
          console.log(`[MCP] Retrying in 2s...`);
    ...
    throw new Error(`mcporter failed after ${maxRetries} attempts: ${lastError.message}`);
  }
};
...
class QueueWorker {
  ...
  async start() {
    ...
    // Initialize Agent Manager using CLI wrapper
    try {
      const testResponse = mcporter.call('agent-manager.agent_pool_status', {});
  ...
  async processJobFile(jobFile) {
    try {
      ...
      if (this.runningAgents.has(jobId)) {
        console.log(`⏭️ Skipping job ${jobId} - already processing with agent ${this.runningAgents.get(jobId).agentName}`);
        return;
      }

      // Move to processing/ immediately to avoid race condition
      const processingFile = path.join(PROCESSING_DIR, path.basename(jobFile));
      if (jobFile !== processingFile) {
        try {
          fs.renameSync(jobFile, processingFile);
          console.log(`📁 Moved ${jobId} to processing/`);
        } catch (e) {
          console.warn(`Failed to move to processing/: ${e.message}`);
        }
      }
      ...
      try {
        ...
        if (idleAgent) {
          ...
          console.warn('⚠️ Agent pool full, waiting...');
      ...
      // Log agent creation
      this.runningAgents.set(jobId, { agentId, status: 'processing', agentName, startTime: Date.now() });
  ...
  async updateQueueStatus(jobId, status, result) {
    try {
      // Cek di pending/ dulu
      let jobFile = path.join(PENDING_DIR, jobId + '.yaml');
      let sourceDir = PENDING_DIR;
      
      // Jika tidak ada di pending/, maybe sedang diproses atau sudah dipindah
      if (!fs.existsSync(jobFile)) {
        // Cek di processing/ (sudah dipindah tapi belum selesai)
        const processingFile = path.join(PROCESSING_DIR, jobId + '.yaml');
        if (fs.existsSync(processingFile)) {
          console.log(`⚠️ Job ${jobId} found in processing/, moving to ${status}`);
          jobFile = processingFile;
          sourceDir = PROCESSING_DIR;
        } else {
          console.warn(`Job file not found in pending/ or processing/: ${jobId}`);
          return; // Sudah dipindah oleh thread lain
        }
      }
      ...
      fs.writeFileSync(tempFile, this.formatJobFile(job));
      fs.renameSync(tempFile, targetFile);

      // Remove from source (pending/ atau processing/)
